...
### 必読ファイル読み込み失敗はhard fail

常時読むもの、phase-localの必読ファイル、dispatch packetで指定された必読ファイルは、全件読めた場合だけ「読了」と扱う。timeout、途中打ち切り、未読、読めなかったファイルが1つでもある場合はhard failであり、そのまま設計・判定・返答へ進まない。

timeoutした場合は、読み込み済みの断片で補完せず、timeoutを延ばして再読込する、対象ファイルを個別に読む、必要なら範囲を分けて読む。それでも読めない場合は、読めていないファイル名と失敗理由を報告して停止する。「必要本文は取れている」「一部だけ抜けたが進める」は禁止する。
必要時だけ追加で読むもの:
...
- clone直後、環境再構築、依存、hook接続、生成物の扱いを確認する場合は `setup.md`

各ファイルの役割:

- `Hello-world.md`: Biz-compilerの現在地を返す最小自己記述。repo-local Skillとops entrypointもここで確認する。構成変更時は同じターンで更新する。
- `setup.md`: clone直後や環境再構築時の初期セットアップ入口。必要ツール、hook接続、生成物、確認コマンドを置く。
...
## Repo-local processing only / global Skill contamination 禁止

Biz-compiler repoで `処理ある？`、`Skillある？`、`orchestratorある？`、`rdra-intakeある？` のように聞かれた場合、原則としてこのrepo内の資産だけを確認する。確認対象は `knowledge/ops/registry.md`、`knowledge/ops/skills/`、`knowledge/ops/orchestrators/`、`knowledge/ops/hooks/`、`template/<phase>/`、repo内script/toolである。

グローバルSkill、外部Skill、plugin由来Skillは、ユーザーが明示的に「グローバルSkillとして使って」「外部Skillを発火して」と依頼した場合を除き、repo-local処理の有無判定、phase設計、phaseテスト、orchestrator検証に使わない。

名前がグローバルSkill一覧に存在しても、Biz-compiler repo内に同名のrepo-local資産がない限り、このrepoの処理として扱わない。たとえば `rdra-intakeって処理ある？` への回答は、まずrepo-local資産の有無を確認し、存在しなければ「このrepoにはない」と答える。

00-entryやphase orchestratorの新規セッションテストでは、グローバルSkill、要求定義Skill、外部intake Skillが発火するとテストが汚染される。テスト時は、repo-local AGENTS / Hello-world / template / output のルールだけで動き、初手の自然な業務相談から入口トリアージが働くかを見る。

新規セッションでBiz-compilerの入口を検査する場合、最初に `rdra-intake`、`Skill`、`orchestrator`、`Biz-compiler`、`00-entry` などのメタ語を投げて通すテストにしない。利用者がいきなり雑な業務相談を投げたときに、repo-local入口が動くかを検査する。

00-entryは、初回発話だけで `entry_gate_result = pass` / `defer` にしてterminal完了扱いしない。資料添付や追加回答などの根拠がなく、質問0の状態なら、`rework` / non-terminal loop として00内の追加質問へ戻す。`defer` はterminalなので、未確認事項を10へ送るための言い訳として質問0完了に使わない。
...
- repo-local Skill作成/改修時は `skill-creator-gate` のproposal、collision check、validation結果
...
- **Hello World は嘘をつかない。** ルート構成、読み込み順、`template/`、`knowledge/`、`output/` の業務ID生成ルール、`setup.md` の環境前提を変えたら、ハロワゲートを通す。
...
- `setup.md` はclone直後の初期セットアップ入口である。hook一覧は複製せず、`knowledge/ops/registry.md` と `knowledge/ops/hooks/README.md` へ誘導する。

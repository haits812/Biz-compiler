...
作業対象がコード、ドキュメント、Skill、ops、templateを変更する場合は、編集前に `impact-orchestrator` で作業の意図、影響、読むもの、完了gateを切る。短い質問回答、読み取りだけ、対象なしのGitHub確認では不要。
...
- `knowledge/ops/` 直下に `.ps1` を置かない。Skill実行体は `knowledge/ops/skills/<skill-name>/`、orchestrator実行体は `knowledge/ops/orchestrators/<orchestrator-name>/` に説明ファイルと同居させる。

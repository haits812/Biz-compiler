...
| `provided` | ユーザーが渡したが、内容確認は10で行う |
| `unavailable` | ない、取れない、アクセスできない |
| `unknown` | 存在有無が不明 |

## Source Candidates

| source_candidate_id | source_type | description | owner_or_location | status | supports | caveat |
|---|---|---|---|---|---|---|
| `SC-001` | `manual/tool/log/screen/output_sample/email/spreadsheet/concept_note/reference/interview` | `<確認する材料>` | `<所在/持ち主>` | `target` | `<何の確認に使うか>` | `<注意>` |

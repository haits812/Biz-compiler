...
switch ($Action) {
  ...
  "approve" {
    ...
    Write-Output "Next: reflect the approved content into its formal target, then run: .\knowledge\ops\pending-review.ps1 applied -Id 'approved/$($item.Category)/$(Split-Path -Leaf $dest)'"
  }

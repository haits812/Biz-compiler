...
| `code-impact` | `knowledge/ops/orchestrators/code-impact/README.md` | メタ土台の関連ファイル候補、SQLite graph index、impact-orchestrator/hello-world-gate連携を扱う時 |
...
このコマンドは、ハロワ更新、検査、`code-impact rebuild`、`git add -A`、日本語commit、`git push`、post-checkを順番に実行する。GitHub/push依頼では、先に `git status --short --branch` で対象有無を見る。対象がなければgate本体を実行せず `対象ないよ。main と origin/main は同期済み。` と返す。
...
## orchestrators/code-impact/

Biz-compilerメタ土台を軽量SQLite graphにし、変更対象から読むべき関連ファイル候補を辿るorchestratorである。安全判定ではなく候補抽出として扱う。

```powershell
.\knowledge\ops\orchestrators\code-impact\code-impact.ps1 rebuild
.\knowledge\ops\orchestrators\code-impact\code-impact.ps1 query -Files "MEMORY.md" -Depth 2 -Format markdown
```

`impact-orchestrator` は `-Files` がある作業登録時に、このquery結果を既定でwork cardに添付する。不要な時だけ `-NoCodeImpact` を付ける。
## hooks/

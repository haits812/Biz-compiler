...
3. ローカル差分がなければ、未push commitの有無を見る。
4. 未push commitがあればpushする。
5. ローカル差分があれば `git add -A` する。
6. 日本語commit messageでcommitする。
7. `git push` する。
8. post-checkする。
...
- `knowledge/ops/registry.md`、hooks、orchestrators、repo-local commands/tools、work locks の追加・変更

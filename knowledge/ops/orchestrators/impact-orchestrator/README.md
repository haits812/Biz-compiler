...
`-NoWrite` を付けると、work cardとlockを書かず、内容だけ出力する。`-NoLock` を付けるとlockを取らない。`-Files` がある register では既定で `code-impact query` を呼び、関連ファイル候補を work card に添付する。不要な時だけ `-NoCodeImpact` を付ける。`-IncludeCodeImpact` は互換用に残すが、現在は付けなくても動く。
...
`-Files` がある register では、既定で `code-impact query` を呼び、depth 2 の関連候補を work card の `Code Impact Candidates` に残す。

`code-impact` の index が無い、または古い場合は、自動で `code-impact.ps1 rebuild` を試してから再queryする。rebuildやqueryが失敗した場合も work card 作成は止めず、同セクションに警告と再実行コマンドを残す。

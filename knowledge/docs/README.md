...
`COMPASS.md` は、会話開始時やsubagent委譲時に必ず渡す heading packet として短く保つ。詳細はこの階層へ分け、共通語彙は初期に読み、それ以外は作業種別に応じて必要なものだけ読む。read routing と subagent 委譲時の必読リストは、このREADMEと `knowledge/docs/meta/subagent-brief.md` を正本にし、`COMPASS.md` へ重複させない。
...
| 用語の意味・語彙境界を深掘りする時 | `knowledge/docs/lexicon.md` | `provenance`、`confidence`、`gate`、`approval`、`validation`、`N-interface` などの定義。通常作業では初期必読 |
...
`COMPASS.md` に残すものは、作業者の思考の向き、価値順位、疑うもの、判断の倒し方である。repoの現在地、読み込み順、read routing、委譲時の必読リスト、ops入口、phase固有手順は `COMPASS.md` に置かない。

`knowledge/docs/` に逃がすものは、メタ土台の棚卸し、用語定義、IR詳細、方法論、事例、評価条件、決定理由である。用語定義の入口である `knowledge/docs/lexicon.md` は、subagent委譲やphase設計の前提語彙を揃えるため通常作業の初期に読む。

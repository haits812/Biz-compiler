...
- `githubあげといて` / `GitHub上げて` / `pushして` / `pushしといて`: まず `git status --short --branch` で対象有無を見る。ローカル差分も未push commitもなく同期済みなら、gate本体は実行せず `対象ないよ。main と origin/main は同期済み。` と返す。対象がある場合だけ、ハロワ更新・検査込みで日本語commitとGitHub pushまで行う。

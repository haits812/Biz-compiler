...
未承認候補の作成・承認だけを別Skillとして分けると、実態は同じ知識整理なのに「どちらを使うのか」が人間側に分かりにくい。特に、`MEMORY.md` を普段メモの入口に戻した後は、pending候補の作成・承認も knowledge curation の一部として扱う方が自然である。

## Decision

Biz-compilerでは、知識整理系Skillを `knowledge-curation` に一本化する。

`pending` は未承認候補の状態名として残すが、未承認候補専用のSkillやフォルダは作らない。

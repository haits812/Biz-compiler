# Work Card: 00-entry finalizeでhandoff実ファイル生成

- work_id: 20260617-223648-00-entry-finalizeでhandoff実ファイル生成
- registered_at: 2026-06-17 22:36:48 +09:00
- kind: template
- state: completed
- executor: codex
- risk: medium
...
00-entryのterminal resultでhandoff.mdを確実にmaterializeし、10の新規セッションへ渡せるファイルを生成する

## Impact

00-entry phase-orchestrator、00 dispatch/review契約、00->10 handoff運用
...
- template/00-entry/_context/dispatch-packet.md
  ...
  - template/10-source-intake/_context/dispatch-checks.md:3
...
- template/00-entry/_context/dispatch-checks.md
  ...
  - template/10-source-intake/_context/dispatch-packet.md:11
...
- status: pending
- close command: `knowledge/ops/orchestrators/impact-orchestrator/impact-orchestrator.ps1 -Complete -WorkCard "knowledge/journal/work/20260617-223648-00-entry-finalizeでhandoff実ファイル生成.md" -Result "..." -Verified "..." -ResidualRisk "..."`
...
- completed_at: 2026-06-17 22:53:39 +09:00
- status: completed
- result: 00-entry terminal resultからhandoff.mdをmaterializeするfinalizeコマンドと、10-source-intake最低入力チェックを追加
- verified: help実行、accept_pass正常系、accept_defer正常系、handoff draft欠落、placeholder残り、pass hard-risk拒否、git diff --check
- residual_risk: 正規のMarkdown構造に依存する軽量検査。将来はschema化が必要
- released_locks: 4

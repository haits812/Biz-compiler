# Work Card: 00-10 route flag本人確認ゲート実装

- work_id: 20260618-170640-00-10-route-flag本人確認ゲート実装
- registered_at: 2026-06-18 17:06:40 +09:00
- kind: template
- state: completed
- executor: codex
- risk: medium
...
20実装前に、00で初期route flagを立て、本人確認状態を含めて10と20へ渡す契約を強化する

## Impact

00-entryと10-source-intakeのphase contract/check/handoff/reviewer、00-10 persona smoke runnerに影響。20本体には入らず、20が読む入口routeの品質を上げる。
...
- template/_shared/internal/runner/replay/run-00-10-persona-smoke.ps1
  ...
  - knowledge/journal/work/20260618-143706-部門横断business-program-personaを00-10テストへ追加.md:23
  - knowledge/journal/work/20260618-143706-部門横断business-program-personaを00-10テストへ追加.md:137
...
- status: pending
- close command: `knowledge/ops/orchestrators/impact-orchestrator/impact-orchestrator.ps1 -Complete -WorkCard "knowledge/journal/work/20260618-170640-00-10-route-flag本人確認ゲート実装.md" -Result "..." -Verified "..." -ResidualRisk "..."`
...
- completed_at: 2026-06-18 17:25:08 +09:00
- status: completed
- result: 00でroute flagを立て、本人確認状態をhandoffへ必須化。10でroute確認を再チェックして20へroute_delta_from_00付きで渡す契約に更新。00/10 reviewerにもroute確認欠落をreworkへ落とすgateを追加。
- verified: PowerShell AST parse OK。Markdown literal newline/control char check OK。00-10 persona smoke 39/39 passed。10-source-intake regression 6/6 passed。p39 negative testで00/10ともRoute Confirmation欠落をask_reworkとして拒否。git diff --check対象tracked差分OK。
- residual_risk: hello-world-gateは現状git add -A/commit/push経路を含むため、このターンでは未実行。既存dirty/untrackedが多いので、push前に別途Hello-world同期と差分整理が必要。
- released_locks: 13
